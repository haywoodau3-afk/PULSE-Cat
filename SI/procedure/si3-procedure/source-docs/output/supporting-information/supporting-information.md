...
![Figure S1. Independent progressive learning curves](figures/si_fig1_independent_learning_curves.svg)

![Figure S2. Independent estimator variants](figures/si_fig2_model_variant_curves.svg)

![Figure S3. Guarded transfer across representation versions](figures/si_fig3_representation_version_bars.svg)

![Figure S4. Reciprocal progressive transfer curves](figures/si_fig4_progressive_transfer_curves.svg)
...
[`si_model_progression_tables.md`](tables/si_model_progression_tables.md).
The corresponding machine-readable tables are
[`si_experiment_inventory.csv`](tables/si_experiment_inventory.csv),
[`si_feature_arm_definitions.csv`](tables/si_feature_arm_definitions.csv),
[`si_representation_progression_ee.csv`](tables/si_representation_progression_ee.csv),
[`si_independent_progressive_model_metrics.csv`](tables/si_independent_progressive_model_metrics.csv),
and [`si_holdout_model_comparison_ee.csv`](tables/si_holdout_model_comparison_ee.csv).
For spreadsheet review, see [`si_model_progression_tables.xlsx`](si_model_progression_tables.xlsx).
...
![Figure S5. Representative P7/CMC-Por pose overlays](figures/si_fig5_pose_overlay_montage.svg)

The full per-substrate SVG views are listed in
[`si_pose_overlay_index.csv`](tables/si_pose_overlay_index.csv).
The corresponding numerical density arrays (`.npz`) are copied into the
raw-data deposit. XYZ coordinate caches remain in the source data tree and are
the only scientific data layer intentionally omitted from this SI package.

## S5. Starting-material and product SMILES

The machine-readable table
[`si_substrate_smiles.csv`](tables/si_substrate_smiles.csv)
...
The complete non-XYZ deposit is organized under [`raw-data/`](raw-data/). Its
[`README.md`](raw-data/README.md) describes the seven scientific categories;
[`raw-data-manifest.csv`](raw-data/raw-data-manifest.csv) and
[`raw-data-manifest.json`](raw-data/raw-data-manifest.json) record the original
source path, byte size, format, scientific level, and SHA-256 digest for every
included file. The checkpoint-level coverage audit is provided in
[`si_completeness_audit.md`](si_completeness_audit.md).
